...
def handle_join(event):
    ...
    line_bot_api.reply_message(
        event.reply_token,
        TextSendMessage(text="👋 تم انضمامي! أرسل `.lurk on` لتفعيل التتبع.")
    )
...
def handle_message(event):
    ...
    if text == ".lurk on":
        ...
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=f"📊 حالة Lurk: {status}\n👀 عدد القراء: {count}"))
